...
namespace HTWebRemoteHost.Devices.Controllers
{
    class NVShieldControl
    {
        ...
        private static void ExecuteCmd(string cmd, string param)
        {
            ...
            try
            {
                string result = $"adb {arguments}".Bash();

                if(!string.IsNullOrEmpty(result.Trim()) && !result.Contains("already connected") && !result.Contains("connected to"))
                {
                    Util.ErrorHandler.SendError(result);

                    if (result.Contains("failed to authenticate to"))
                    {
